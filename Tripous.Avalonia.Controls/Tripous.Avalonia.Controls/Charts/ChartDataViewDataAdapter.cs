...
/// <summary>
/// Adapts a <see cref="DataView"/> to the <see cref="IChartDataAdapter"/> contract.
/// </summary>
public class ChartDataViewDataAdapter: IChartDataAdapter, IDisposable
{
    // ● private fields
    ...
    List<ChartSourceField> CreateSourceFields()
    {
        if (fView.Table == null)
            return new List<ChartSourceField>();

        return fView.Table.Columns
            .Cast<DataColumn>()
            .Select(Column => ChartFieldRules.CreateSourceField(Column.ColumnName, string.IsNullOrWhiteSpace(Column.Caption) ? Column.ColumnName : Column.Caption, Column.DataType))
    ...
    // ● constructor
    /// <summary>
    /// Initializes a new instance of the <see cref="ChartDataViewDataAdapter"/> class.
    /// </summary>
    /// <param name="View">The source data view.</param>
    public ChartDataViewDataAdapter(DataView View)
    {
