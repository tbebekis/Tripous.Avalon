...
public class ListProvider<T>: IDataProvider where T: class, INotifyPropertyChanged, new()
{
    ...
    void List_ListChanged(object Sender, ListChangedEventArgs e)
    {
        ...
        if (e.ListChangedType == ListChangedType.ItemAdded && e.NewIndex >= 0 && e.NewIndex < fList.Count)
            Subscribe(fList[e.NewIndex]);
    ...
    public ListProvider(IList<T> List)
    {
        ...
        if (fList is IBindingList BindingList)
            BindingList.ListChanged += List_ListChanged;
        if (fList is INotifyCollectionChanged Collection)
            Collection.CollectionChanged += List_CollectionChanged;
    }
    ...
    /// <summary>
    /// Occurs when list items change through provider operations.
    /// </summary>
