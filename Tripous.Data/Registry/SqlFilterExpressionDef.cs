...
/// <summary>
/// A filter expression,
/// <para>e.g. <c>[[string:Customer Name]]</c></para>
/// <para>See the <c>SqlFilterExpressions.txt</c> file for the filter syntax.</para>
/// </summary>
public class SqlFilterExpressionDef
{
    ...
    private void Parse()
    {
        ...
        try
        {
            ...
            if (!TryParseFilterType(parts[0], out SqlFilterExpressionType filterType))
            {
            ...
            {
                case SqlFilterExpressionType.Date:
                    ParseDate(parts);
                    break;

                case SqlFilterExpressionType.Lookup:
                    ParseLookup(parts);
                    break;

                case SqlFilterExpressionType.Enum:
                    ParseEnum(parts);
                    break;

                case SqlFilterExpressionType.String:
                case SqlFilterExpressionType.Integer:
                case SqlFilterExpressionType.Decimal:
                    ParseSimple(parts);
                    break;

                default:
                    AddError($"Unsupported filter type '{ExpressionType}'.");
                    break;
            }
        }
        catch (Exception e)
    ...
    private void ParseSimple(string[] parts)
    {
        ...
        if (parts.Length != 2)
        {
            AddError($"{ExpressionType} filter requires exactly one label.");
            return;
        }
    ...
    private bool TryParseFilterType(string token, out SqlFilterExpressionType filterType)
    {
        ...
        {
            filterType = SqlFilterExpressionType.Date;
            return true;
        }

        if (EqualsIgnoreCase(token, "int") || EqualsIgnoreCase(token, "integer"))
        {
            filterType = SqlFilterExpressionType.Integer;
            return true;
        }

        if (EqualsIgnoreCase(token, "dec") || EqualsIgnoreCase(token, "decimal"))
        {
            filterType = SqlFilterExpressionType.Decimal;
            return true;
        }

        if (EqualsIgnoreCase(token, "lookup"))
        {
            filterType = SqlFilterExpressionType.Lookup;
            return true;
        }

        if (EqualsIgnoreCase(token, "enum"))
        {
            filterType = SqlFilterExpressionType.Enum;
            return true;
        }
    ...
    // ● properties
    /// <summary>
    /// The raw tag, e.g. <c>[[date:A_Date]]</c>
    /// </summary>
    ...
    public string LookUpSelectSqlText
    {
        get
        {
            ...
            if (ExpressionType == SqlFilterExpressionType.Lookup)
            {
    ...
    /// </summary>
    [JsonIgnore]
    public SqlFilterExpressionType ExpressionType { get; private set; }
    /// <summary>
