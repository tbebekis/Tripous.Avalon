...
public class TableSet
{
    ...
    void Select_DoDetail(MemTable MasterTable, MemTable Detail)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Detail.SqlStatements.SelectSql))
        {
            ...
            if ((MasterTable.Rows.Count > 0) && (MasterTable.Columns.Contains(Detail.MasterField)))
            {
                ...
                string[] KeyValuesList = MasterTable.GetKeyValuesList(Detail.MasterField, 100, false);
    ...
    private void PostDeletes()
    {
        ...
        void DeleteTable (MemTable Table)
        {
            ...
            if (Source != null)
            {
                ...
                /* delete the rows */
                string[] KeyValuesList = Source.GetKeyValuesList(Table.KeyField, 100, true);
        ...
        while (Level <= MaxDetailLevel)
        {
            foreach (MemTable Table in TableTree)
            {
                if (Table.Level == Level && !Table.IsEmpty && Table.Columns.Contains(Table.KeyField))
                {
                    ...
                    foreach (DataRow Row in Table.Rows)
                    {
                        if (Row.RowState == DataRowState.Added && !Row.IsNull(Table.KeyField))
                        {
    ...
    /// </summary>
    public bool Select(object RowId)
    {
    ...
    public void ProcessCancel()
    {
        if (IsInsert)
            ...
            Select(TopTable.Rows[0][TopTable.KeyFields[0]]);
    }    
